...
async function disponibilidadSillas(req,res){
    ...
    try {
        ...
        const ultima_reserva_usuario = await Reserva.reserva.findOne({
            where: {
                fk_persona: pk_numero_documento
            },
            order: [
...
async function crear_reserva(req, res) {
    try {
        ...
        const newReserva = await Reserva.reserva.create({
            v_estado: 'en proceso',
            t_inicioreserva: date,
            fk_persona: req.pk_numero_documento['pk_numero_documento']
        });
        res.json({
            data: newReserva
        });
    ...
    try {
        ...
        if (reserva) {
            ...
            if (tiempo_limite > Date.now().valueOf()) {
                ...
                if (silla_reservada) {
                    if (pk_numero_documento == reserva.fk_persona) {
    ...
    try {
        ...
        if(reserva){
            ...
            if (tiempo_limite > Date.now().valueOf()) {
                ...
                if(snack_reservado){
                    if (req.pk_numero_documento['pk_numero_documento'] == reserva.fk_persona) {
